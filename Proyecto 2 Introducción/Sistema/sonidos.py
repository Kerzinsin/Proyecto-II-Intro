...
class GestorSonidos:
    def __init__(self):
        ...
        # Carpeta para archivos de sonido (opcional)
    ...
    def _generar_beep(self, frecuencia, duracion, sample_rate):
        ...
        with wave.open(temp_file.name, 'w') as wav_file:
            ...
            for i in range(frames):
                # Onda sinusoidal con envolvente
                ...
                # Convertir a formato de 16 bits estéreo
                sample = max(-32768, min(32767, sample))
                # Empaquetar como bytes (little-endian, signed short)
                wav_file.writeframes(struct.pack('<hh', sample, sample))
        
        # Cargar el sonido desde el archivo WAV
        sound = pygame.mixer.Sound(temp_file.name)
        
        # Limpiar el archivo temporal después de cargarlo
        try:
    ...
    def reproducir_musica(self, loop=True):
        ...
        # Generar música de fondo simple
        if self.musica_actual is None:
            self.musica_actual = self._generar_musica_fondo()
        
        # Reproducir el sonido directamente (no usar mixer.music)
        self.musica_actual.set_volume(self.volumen_musica)
        if loop:
            self.musica_actual.play(-1)  # -1 significa loop infinito
        else:
    ...
    def _generar_musica_fondo(self):
        ...
        # Acordes simples para ambiente
        notas = [261, 329, 392]  # Do, Mi, Sol (acorde mayor)
        ...
        with wave.open(temp_file.name, 'w') as wav_file:
            ...
            for i in range(frames):
                ...
                # Agregar un poco de variación
                wave_value += math.sin(2 * math.pi * 110 * t) * 0.1  # Nota baja de fondo
